...
const Change_banner = ({ visible, onClose }) => {
  ...
  function updateLogo() {
    ...
    if (auth.currentUser) {
      auth.currentUser.getIdToken().then(async (idToken) => {
        ...
        const fileInput = new FormData(document.getElementById("emailSignUp"));
        fetch(
          ...
          .then((data) => {
            ...
            if (data.status === "OK") {
              const storageRef = ref(storage, data.data.photoPathFirestore);
              ...
              setClicked(false);
              document.getElementById("container").click();
              alert("Error occurred while changing photo with status != 200");
              alert(data.message);
            }
          })
          .catch((err) => {
