              ...
              getDownloadURL(storageRef)
                .then(async function (url) {
                  ...
                  const logoPromises = comms.map((comm) => {
                    const storageRef = ref(storage, comm.organization.logo);
                    return getDownloadURL(storageRef);
                  });
                  const logoPromisesResolved = await Promise.all(logoPromises);
                  const updatedComms = comms.map((comm, index) => {
                    return {
                      ...comm,
      ...
      <div className="w-full bg-[#0f1035] text-white flex justify-between ">
        ...
        <div className="flex justify-end items-center text-2xl sm:text-2xl md:text-3xl mr-1">
          ...
          <div className="flex mx-2 sm:mx-5 lg:mx-8 items-center">
            ...
            <div className="text-3xl hidden md:flex">{profile.name}</div>
        ...
        >
          <Link to="/user/profile" className="p-4 hover:scale-100 duration-300">
            View Profile
          </Link>
          <a
                  ...
                  communities
                    ...
                    .map((community, key) => (
                      <a
                        key={key}
                        href={
                          "/org/profile/" + community.organization.firebaseId
                        }
                      >
            ...
            <div className="border-black border-[1px] flex flex-col gap-5 bg-[#E1E5CD] pt-1">
              {campaigns?.length > 0 ? (
                          ...
                          <div className="flex justify-between items-baseline gap-3 ">
                            <div>
                              ...
                              <div className="text-[0.7rem]">
                                {campaign.registeredUsersCount} People joined
                              </div>
                            </div>
                ...
                <div className="flex justify-center items-center w-fit p-20 rounded-lg">
                  <p className="text-black">No upcoming campaigns!</p>
                </div>
              )}
            </div>
            <button
